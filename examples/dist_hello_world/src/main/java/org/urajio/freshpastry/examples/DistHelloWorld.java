...
import org.urajio.freshpastry.rice.pastry.dist.DistPastryNodeFactory;
import org.urajio.freshpastry.rice.pastry.socket.SocketPastryNodeFactory;
import org.urajio.freshpastry.rice.pastry.standard.IPNodeIdFactory;
    ...
    private static void doIinitstuff(String[] args, Environment env) {
        ...
        for (int i = 0; i < args.length; i++) {
            if (args[i].equals("-silent") && i + 1 < args.length) {
                env.getParameters().setInt("loglevel", Level.ERROR.toInt());
                break;
            }
        }
        ...
        for (int i = 0; i < args.length; i++) {
            if (args[i].equals("-verbose") && i + 1 < args.length) {
                env.getParameters().setInt("loglevel", Level.DEBUG.toInt());
                break;
            }
        }
