...
import org.urajio.freshpastry.org.mpisws.p2p.transport.sourceroute.Forwarder;
...
public class RendezvousTransportLayerImpl<Identifier, HighIdentifier extends RendezvousContact> implements
    ...
    private final static Logger logger = LoggerFactory.getLogger(RendezvousTransportLayerImpl.class);
    // *************Pilot Sockets (used to connect leafset members) ******************
    // *************** outgoing Pilots, only used by NATted nodes ********************
    final Map<HighIdentifier, OutgoingPilot> outgoingPilots = new HashMap<>();
    // listener
    ...
    public SocketRequestHandle<Identifier> openSocket(final Identifier i, final SocketCallback<Identifier> deliverSocketToMe, final Map<String, Object> options) {
        ...
        final SocketRequestHandle<Identifier> handle = new SocketRequestHandleImpl<>(i, options);
        ...
        if (contact == null || contact.canContactDirect() || contactDirectStrategy.canContactDirect(contact)) {
            if (logger.isDebugEnabled()) {
                ...
                logger.debug("openSocket(" + i + "," + deliverSocketToMe + "," + options + ") contact:" + s);
            }
            ...
            if (options.containsKey(OPTION_USE_PILOT)) {
                ...
                logger.debug("Opening socket to " + contact + " OPTION_USE_PILOT->" + middleMan);
    ...
    private void openSocketUsingRouting(final HighIdentifier contact,
        ...
        logger.debug("opening a socket using routing to " + contact);
        ...
        rendezvousStrategy.openChannel(contact, localNodeHandle, localNodeHandle, uid, new Continuation<Integer, Exception>() {
            ...
            public void receiveException(Exception exception) {
                logger.info("openSocket(" + contact + "," + deliverSocketToMe + "," + options + ")", exception);
                deliverSocketToMe.receiveException(handle, exception);
            }
        }, options);
    }
    ...
    private boolean openSocketUsingPilotFinder(HighIdentifier contact,
        ...
        if (middleMan == null) {
            ...
            // use middleman
            logger.debug("opening a socket to " + contact + " via " + middleMan);
            openSocketViaPilot(contact, middleMan, handle, deliverSocketToMe, options);
            return true;
        }
    }
    ...
    protected boolean openSocketUsingPilotToMe(HighIdentifier contact,
        ...
        if (incomingPilots.containsKey(contact)) {
            ...
            logger.debug("Opening socket to firewalled node that I have a pilot to: " + contact + " uid:" + uid);
            try {
    ...
    protected void openSocketViaPilot(
        ...
        final int uid = random.nextInt();
        logger.debug("openSocketViaPilot<" + uid + ">(" + dest + "," + middleMan + "," + handle + "," + deliverSocketToMe + "," + options + ")");
                        ...
                        public void receiveSelectResult(P2PSocket<Identifier> socket, boolean canRead, boolean canWrite) throws IOException {
                            ...
                            if (!writeBuffer.hasRemaining()) {
                                ...
                                if (response == CONNECTION_RESPONSE_SUCCESS) {
                                    logger.debug("success in openSocketViaPilot<" + uid + ">(" + dest + "," + middleMan + "," + handle + "," + deliverSocketToMe + "," + options + ")");
                                    deliverSocketToMe.receiveResult(handle, socket);
                                    return;
                                }
    ...
    public void incomingSocket(P2PSocket<Identifier> s) throws IOException {
        ...
        new P2PSocketReceiver<Identifier>() {
            ...
            public void receiveSelectResult(P2PSocket<Identifier> socket, boolean canRead, boolean canWrite) throws IOException {
                logger.debug("incomingSocket(" + socket + ").rSR(" + canRead + "," + canWrite + ")");
                ...
                switch (socketType) {
                    case NORMAL_SOCKET:
                        logger.debug("incomingSocket(" + socket + ").rSR(" + canRead + "," + canWrite + "):NORMAL");
    ...
    protected void readConnectHeader(P2PSocket<Identifier> socket) throws IOException {
        logger.debug("readConnectHeader(" + socket + ")");
        ...
        P2PSocketReceiver<Identifier> receiver = new P2PSocketReceiver<Identifier>() {
            ...
            public void receiveSelectResult(P2PSocket<Identifier> socket, boolean canRead, boolean canWrite) throws IOException {
                ...
                try {
                    ...
                    logger.debug("readConnectHeader(" + socket + "," + target + "," + opener + "," + uid + ")");
                    // TODO: make a timeout for this structure...
                    putConnectSocket(opener, target, uid, socket);

                    if (incomingPilots.containsKey(target)) {
                        logger.debug("I'm the rendezevous for " + opener + " to " + target + " and I have a pilot.");
                        ...
                        logger.info("I'm the rendezevous for " + opener + " to " + target + " and I don't have a pilot.");
            ...
            public void receiveException(P2PSocket<Identifier> socket, Exception ioe) {
                ...
                logger.warn("error in readConnectHeader(" + socket + ") closing.", ioe);
                socket.close();
            }
        };
    ...
    protected void readAcceptHeader(P2PSocket<Identifier> acceptorSocket) throws IOException {
        logger.debug("readAcceptHeader(" + acceptorSocket + ")");
                ...
                try {
                    ...
                    if (opener.equals(localNodeHandle)) {
                        ...
                        if (deliverSocketToMe == null) {
                            logger.warn("Got accept socket to me, that I'm not expecting: t:" + target + " o:" + opener + " uid:" + uid + " " + acceptorSocket);
                        ...
                        new ByteWriter(CONNECTION_RESPONSE_SUCCESS, new Continuation<P2PSocket<Identifier>, Exception>() {
                            ...
                            public void receiveResult(P2PSocket<Identifier> result) {
                                ...
                                deliverSocketToMe.a().receiveResult(deliverSocketToMe.b(), new SocketWrapperSocket<>(result.getIdentifier(), result, errorHandler, OptionsFactory.merge(deliverSocketToMe.b().getOptions(), result.getOptions())));
                            }
                        }).receiveSelectResult(acceptorSocket, false, true);
                        return;
                    }

                    logger.debug("readAcceptHeader(" + acceptorSocket + "," + target + "," + opener + "," + uid + ")");
                    ...
                    if (connectorSocket == null) {
                        logger.debug("writing failed bytes in readAcceptHeader(" + acceptorSocket + "," + target + "," + opener + "," + uid + ")");
                        ...
                        P2PSocketReceiver<Identifier> acceptorFailed = new ByteWriter(CONNECTION_RESPONSE_FAILURE,
                                new Continuation<P2PSocket<Identifier>, Exception>() {
                                    ...
                                    public void receiveException(Exception exception) {
                                        logger.warn("Error writing failed bytes in readAcceptHeader(" + acceptorSocket + "," + target + "," + opener + "," + uid + ")", exception);
                        ...
                        logger.debug("writing success bytes in readAcceptHeader(" + acceptorSocket + "," + target + "," + opener + "," + uid + ")");
                        ...
                        P2PSocketReceiver<Identifier> connectorSuccess = new ByteWriter(CONNECTION_RESPONSE_SUCCESS,
                                new Continuation<P2PSocket<Identifier>, Exception>() {
                                    public void receiveResult(P2PSocket<Identifier> result) {
                                        ...
                                        logger.debug("Connector socket complete, setting up forwarding. readAcceptHeader(" + acceptorSocket + "," + target + "," + opener + "," + uid + ")");

                                        forwardSockets.setA(result);
                                        if (forwardSockets.b() != null) {
                                            createForwarder(forwardSockets.a(), forwardSockets.b(), opener, target, uid);
                                        }
                                    }
                        ...
                        P2PSocketReceiver<Identifier> acceptorSuccess = new ByteWriter(CONNECTION_RESPONSE_SUCCESS,
                                new Continuation<P2PSocket<Identifier>, Exception>() {
                                    public void receiveResult(P2PSocket<Identifier> result) {
                                        // done, set up the forwarder
                                        logger.debug("Acceptor socket complete, setting up forwarding. readAcceptHeader(" + acceptorSocket + "," + target + "," + opener + "," + uid + ")");
    ...
    protected void putExpectedIncomingSocket(HighIdentifier contact, int uid,
        ...
        logger.debug("putExpectedIncomingSocket(" + contact + "@" + System.identityHashCode(contact) + "," + uid + "," + deliverSocketToMe + "," + requestHandle + ")");
    ...
    protected Tuple<SocketCallback<Identifier>, SocketRequestHandle<Identifier>> removeExpectedIncomingSocket(HighIdentifier target, int uid) {
        logger.debug("removeExpectedIncomingSocket(" + target + "@" + System.identityHashCode(target) + "," + uid + ")");
        ...
        if (one == null) {
            if (logger.isDebugEnabled()) {
                ...
                logger.debug("removeExpectedIncomingSocket(" + target + "@" + System.identityHashCode(target) + "," + uid + ") had no first level entry for target" + s);
            }
            return null;
        }
        ...
        if (ret == null) {
            if (logger.isDebugEnabled()) {
                ...
                logger.debug("removeExpectedIncomingSocket(" + target + "@" + System.identityHashCode(target) + "," + uid + ") had no uid for target" + s);
            }
        } else {
            one.remove(uid);
        }
    ...
    protected void createForwarder(P2PSocket<Identifier> a, P2PSocket<Identifier> b, HighIdentifier connector, HighIdentifier acceptor, int uid) {
        logger.debug("createForwarder(" + a + "," + b + "," + connector + "," + acceptor + "," + uid + ")");
        new Forwarder<>(null, a, b);
    }
    ...
    public void putConnectSocket(HighIdentifier requestor, HighIdentifier target, int uid, P2PSocket<Identifier> socket) {
        ...
        if (three != null) {
            ...
            logger.warn("error in storeConnectSocket() there is already a connector with the same UID!!!, dropping the new one.  Old:" + three + " new:" + socket);
            socket.close();
            return;
        }
    ...
    public void openChannel(HighIdentifier requestor, HighIdentifier middleMan, int uid) {
        logger.info("openChannel(" + requestor + "," + middleMan + "," + uid + ")");
    ...
    protected void openAcceptSocket(final HighIdentifier requestor, final HighIdentifier middleMan, final int uid) {
        logger.debug("openAcceptSocket(" + requestor + "," + middleMan + "," + uid + ")");
...
//    }
        ...
        try {
            ...
            logger.warn("Error serializing in openAcceptSocket(" + requestor + "," + middleMan + "," + uid + ")", ioe);
            return;
        }
                    ...
                    new P2PSocketReceiver<Identifier>() {
                        ...
                        public void receiveSelectResult(P2PSocket<Identifier> socket,
                            ...
                            if (writeBuffer.hasRemaining()) {
                                ...
                                long bytesWritten = socket.write(writeBuffer);
                                if (bytesWritten < 0) {
                                    logger.warn("Channel closed in openAcceptSocket(" + requestor + "," + middleMan + "," + uid + ")");
                                    return;
                                }
                                if (writeBuffer.hasRemaining()) {
                                    socket.register(false, true, this);
                            ...
                            if (!writeBuffer.hasRemaining()) {
                                ...
                                byte response = readBuffer.get();
                                if (response == CONNECTION_RESPONSE_SUCCESS) {
                                    logger.debug("success in openAcceptSocket(" + requestor + "," + middleMan + "," + uid + ")");
                                    callback.incomingSocket(socket);
                                    return;
                                }
                                logger.warn("Failed to connect in openAcceptSocket(" + requestor + "," + middleMan + "," + uid + ")");
                            }
                        }

                        public void receiveException(P2PSocket<Identifier> s,
                                                     Exception ex) {
                            logger.warn("Failure opening socket in openAcceptSocket(" + requestor + "," + middleMan + "," + uid + ")", ex);
                        }
                    }.receiveSelectResult(sock, false, true);
                } catch (IOException ioe) {
                    logger.warn("Exception in openAcceptSocket(" + requestor + "," + middleMan + "," + uid + ")", ioe);
                }
            }
    ...
    public void messageReceivedFromOverlay(HighIdentifier i, ByteBuffer m, Map<String, Object> options) throws IOException {
        logger.debug("messageReceivedFromOverlay(" + i + "," + m + "," + options + ")");
        ...
        if (high == null) {
            ...
            if (high.canContactDirect() || contactDirectStrategy.canContactDirect(high)) {
                ...
                if (ephemeral != null && responseStrategy.sendDirect(ephemeral, m, options)) {
                    ...
                    logger.debug("Sending directly on ephemeral " + ephemeral + " for " + high);
                    responseStrategy.messageSent(ephemeral, m, options);
                    return tl.sendMessage(ephemeral, m, deliverAckToMe, options);
                }
                logger.debug("Not sending directly on ephemeral " + ephemeral + " for " + high);
    ...
    public SocketRequestHandle<HighIdentifier> openPilot(final HighIdentifier i,
        ...
        synchronized (outgoingPilots) {
            if (outgoingPilots.containsKey(i)) {
                logger.debug("openPilot(" + i + ") already had one.");
    ...
    abstract class AbstractPilot extends TimerTask implements P2PSocketReceiver<Identifier> {
        ...
        protected void write() throws IOException {
            ...
            logger.debug(this + " wrote " + ret + " bytes of " + queue.getFirst());
            if (ret < 0) cancel();
    ...
    class OutgoingPilot extends AbstractPilot implements SocketRequestHandle<HighIdentifier> {
        ...
        public void setSocket(P2PSocket<Identifier> socket) {
            ...
            this.socket = socket;
            logger.debug(this + " success opening outgoing pilot");
            try {
        ...
        protected void read() {
            try {
                ...
                switch (msgType) {
                    case PILOT_PONG:
                        logger.debug(this + " received pong");
                        sib.clear();
                    ...
                    case PILOT_REQUEST:
                        ...
                        int uid = sib.readInt();
                        logger.debug("Received socket request: requestor:" + requestor + " middleman:" + i + " uid:" + uid);
    ...
    class IncomingPilot extends AbstractPilot {
        ...
        protected void requestSocket(HighIdentifier requestor, int uid) throws IOException {
            logger.debug("Requesting socket from: " + i + "requestor:" + requestor + " uid:" + uid);
        ...
        protected void read() throws IOException {
            if (i == null) {
                ...
                try {
                    ...
                    logger.debug("Received incoming Pilot from " + i);
                } catch (InsufficientBytesException ibe) {
                    socket.register(true, false, this);
                    return;
                }
            ...
            try {
                ...
                if (msgType == PILOT_PING) {
                    logger.debug(this + " received ping");
                    sib.clear();
        ...
        public void receiveException(P2PSocket<Identifier> socket, Exception ioe) {
            if (i != null) {
                logger.debug("Shutdown of incoming pilot " + socket);
